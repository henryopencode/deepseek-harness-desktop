...
**Expose installer progress over the Host API.** This would add a second progress protocol to a detached updater whose durable outcome is the active version. A bounded wait and version polling cover the required lifecycle without adding model- or session-visible state.
